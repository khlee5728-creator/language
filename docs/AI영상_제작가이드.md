...
8. **자막 표준 (2026-08-28 확정)** — 텍스트 채널은 역할별로 셋:
   ...
   - **한글 병기(2026-09-12, v6)**: 자막·think 큐마다 `<span class="ko">의역 한 줄</span>`을 원문 뒤(`.gl` 앞)에 둔다.
     화면은 영어(키워드 앰버) → 한글(14px, 연한 톤) → `.gl` 주석 순. 우상단 '가 한국어' 토글(`.hide-ko`)로 한글만 숨김.
     think 말풍선은 자막과 같은 반투명 다크(rgba 16,15,12 / .62)로 통일해 화면 가림을 줄인다.
     릴스 빌더(`reels/build_reels.py`)는 덱의 `.ko`를 번역 원본으로 읽으므로 번역 수정은 덱에서만 한다.
