...
- **시안은 반드시 기준 프레임을 참조 이미지로** 생성(P1: W2 시작프레임 job `720de8f0-51c2-4d62-b63c-37cbdc5845ed` / P2: W9 시안 job `8e15f312-…`). 참조 없이 만들면 캐릭터가 달라진다(W11 사고 전례).
...
- **한글 병기(v6)**: 자막·think 큐마다 `<span class="ko">의역 한 줄</span>`을 원문 뒤(.gl 앞)에 둔다. 우상단 '가 한국어' 토글(.hide-ko)로 함께 숨겨짐. 릴스 빌더는 덱의 `.ko`를 번역 원본으로 읽으므로 번역은 덱에서만 고친다. think 말풍선은 자막과 같은 반투명 다크(화면 가림 최소화).
- 노출 부족 시 `data-slow`(슬로모) — 완전 정지(hold)는 금지. 대사 길이∝노출: 최소 max(1.6s, 글자수÷12s).
  마지막 think 펀치라인은 `data-end`를 영상 길이보다 크게 두어 재생 종료 후 마지막 프레임에도 남긴다(v7, W3·W5). 말장난은 `<b>`로 강조(think 안의 `<b>`도 앰버).
